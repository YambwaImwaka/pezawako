...
class PurchasePageState extends State<PurchasePage> {
  ...
  Card _buildConnectionCheckTile() {
    if (_loading) {
      return Card(
          child: ListTile(title: Text(context.lwTranslate.tryingToConnect)));
    }
    ...
    if (!_isAvailable) {
      children.addAll(<Widget>[
        const Divider(),
        ListTile(
          title: Text(context.lwTranslate.notConnected,
    ...
    if (_notFoundIds.isNotEmpty && configItem('debug')) {
      productList.add(
        ListTile(
          title: Text(
            context.lwTranslate
                .notFoundedProductsNotFound(_notFoundIds.join(", ")),
    ...
    productList.addAll(_products.map(
      (ProductDetails productDetails) {
        ...
        if (creditPackage.isEmpty) {
          return ListTile(
            title: Text(context.lwTranslate.packageNotFound),
          );
        }
  ...
  Card _buildConsumableBox() {
    if (_loading) {
      return Card(
          child: ListTile(
              leading: const CircularProgressIndicator(),
              title: Text(context.lwTranslate.fetchingConsumables)));
    }
    if (!_isAvailable) {
      return const Card();
    }
